...
int convertStringToUInt(char input[], unsigned int *output) {
	...
	for (size_t i = 0; input[i] != '\0'; i++) {
		if (input[i] >= '0' && input[i] <= '9') {
			//Wert an Adresse von output wird mit 10 multipliziert um dezimal um eine Stelle nach links zu shiften
			//Vom i-ten Char wird 0x30 bzw '0' abgezogen -> string 3 entspricht 0x33. 0x30 abgezogen entspricht 3.
